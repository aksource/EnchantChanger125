			 ...
			 else if(enchOnItem != null)//extract enchantment from Item
			 {
				 ...
				 for(int i=0;i<8;i++)
				 {
					 if(i < this.ItemEnchList.size())
					 {
						 ...
						 this.materializeResult.setInventorySlotContents(i+1, new ItemStack(mod_EnchantChanger.MateriaID, 1, var1Int));
					 }
					 else
						 break;
				 }
...
//			 ModLoader.setPrivateValue(NBTTagCompound.class, nbt, "tagMap", map);
	 ...
	 public Enchantment EnchKind(int materiaDamage)
	 {
		 ...
		 else if(materiaDamage <= materiamax + EcItemMateria.MagicMateriaNumMax)
		 {
			 int magic = (materiaDamage - materiamax - 1) / maxlv;
			 switch(magic)
			 {
			 case 0:return Enchantment.enchantmentsList[mod_EnchantChanger.EnchantmentMeteoId];
			 case 1:return Enchantment.enchantmentsList[mod_EnchantChanger.EndhantmentHolyId];
			 case 2:return Enchantment.enchantmentsList[mod_EnchantChanger.EnchantmentTelepoId];
			 case 3:return Enchantment.enchantmentsList[mod_EnchantChanger.EnchantmentFloatId];
			 case 4:return Enchantment.enchantmentsList[mod_EnchantChanger.EnchantmentThunderId];
			 default :return Enchantment.enchantmentsList[0];
			 }
		 }
		 else if(mod_EnchantChanger.ExtraEnchantIdArray.size() > 0)
		 {
			 int ExtraKind = (materiaDamage - (materiamax + EcItemMateria.MagicMateriaNumMax) - 1) / maxlv;
			 return Enchantment.enchantmentsList[mod_EnchantChanger.ExtraEnchantIdArray.get(ExtraKind)];
		 }
		 else
		 {
	 ...
	 public int EnchLv(int materiaDamage)
	 {
		 ...
		 {
			 return ((materiaDamage - (materiamax + EcItemMateria.MagicMateriaNumMax)) % maxlv == 0)? maxlv: (materiaDamage - (materiamax + EcItemMateria.MagicMateriaNumMax)) % maxlv;
		 }
		 else
		 {
			 return  0;
	 ...
	 public boolean CheckLvCap(ItemStack par1ItemStack, ItemStack par2ItemStack)
	 {
		 if(mod_EnchantChanger.LevelCap)
		 {
			 ...
			 else if(par1ItemStack.getItemDamage() > materiamax + EcItemMateria.MagicMateriaNumMax && mod_EnchantChanger.ExtraEnchantIdArray.size() > 0)
			 {
				 int var1=(par1ItemStack.getItemDamage() -(materiamax + EcItemMateria.MagicMateriaNumMax)-1) / maxlv;
				 int var2 = par1ItemStack.getItemDamage() % maxlv;
				 return (var2 <= Enchantment.enchantmentsList[mod_EnchantChanger.ExtraEnchantIdArray.get(var1)].getMaxLevel())?true:false;
			 }
			 else
			 {
				 return true;
