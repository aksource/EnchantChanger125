...
public class EcContainerPortableEnchantment extends Container
{
    ...
    public EcContainerPortableEnchantment(InventoryPlayer par1InventoryPlayer, World par2World, int par3, int par4, int par5)
    {
        ...
        this.posY = par4;
        this.posZ = par5;
        this.addSlot(new EcSlotPortableEnchantment(this, this.tableInventory, 0, 25, 47));
        int var6;
        ...
        if (par1IInventory == this.tableInventory)
        {
            ...
            if (var2 != null && var2.isItemEnchantable())
            {
                ...
                if (!this.worldPointer.isRemote)
                {
					...
					*/
                    InventoryPlayer playerinventory = ModLoader.getMinecraftInstance().thePlayer.inventory;
                    for(int i=0;i<36;i++)
                    {
                    	if(playerinventory.getStackInSlot(i) != null && playerinventory.getStackInSlot(i).itemID ==Block.bookShelf.blockID)
                    	{
                    		var3 +=playerinventory.getStackInSlot(i).stackSize;
                    	}
                    }
